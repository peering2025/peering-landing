...
const TOPIC_POOL = [
  ...
  // ── 실용 노하우 (시간표·업무 관련) ──────────────────────────────────────────
  {
    hint: 'IEP 개별화교육계획 작성 시 시간표 연계 노하우',
    category: '팁 & 노하우',
  },
  {
    hint: '특수교육 협력교수 시간표 편성 시 주의사항과 성공 전략',
    category: '팁 & 노하우',
  },
  {
    hint: '특수교사 업무 경감을 위한 디지털 자동화 도구 비교',
    category: '팁 & 노하우',
  },
  {
    hint: '특수학급 신설 첫 해 시간표 구축 A to Z',
    category: '팁 & 노하우',
  },
  {
    hint: '특수교육 보조인력 공정 배분을 위한 기준표 만들기',
    category: '팁 & 노하우',
  },
]
...
async function main() {
  ...
  // 4. 주제 힌트 4개 랜덤 선택 (카테고리 다양성 확보)
  const selectedTopics = shuffle(TOPIC_POOL).slice(0, 4)
  const selectedTopic = selectedTopics[0] // 메인 주제는 첫 번째
...

{
  ...
  "title": "한국어 제목 (45자 이내, 정보성·뉴스성이 느껴지는 제목)",
  "title_en": "English title (65 chars max, informative tone)",
  "date": "${dateStr}",
  "category": "선택한 주제의 카테고리 (예: 해외 연구 동향 / 에듀테크 / 국내 교육 이슈 / 현장 사례 / 교육 정책·제도 / 팁 & 노하우)",
